"""Training script for BPELM (BPE baseline) on WikiText-2.

Run on the training box (RTX 2060):
    python3 training/train_bpe.py

For a fair comparison with train_bitmap.py, use matched params:
    --d_model 256 --n_layers 6 --n_heads 4 --tie_head
...
from models import BPELM, BPEConfig
from pixenc.bpe import train_bpe, load_bpe
from training.data import (
    load_text, bpe_encode, sample_window_batch, eval_loss_bpe,
...
def parse_args():
    p = argparse.ArgumentParser(description="Train BPELM (BPE baseline) on WikiText-2")
    ...
    p.add_argument("--tie_head", action="store_true", default=True,
                   help="tie output head to embedding (default on for BPE baseline)")
    p.add_argument("--no_tie_head", dest="tie_head", action="store_false")

    # BPE
    p.add_argument("--bpe_vocab",  type=int, default=8000, help="BPE vocab size")
    p.add_argument("--bpe_model",  type=str, default=None,
                   help="path to existing BPE model dir — skip training if provided")
    p.add_argument("--bpe_corpus", type=str, default=None,
                   help="corpus to train BPE on (default: same as --train_file)")
    ...
    p.add_argument("--save_dir",    type=str, default="artifacts/checkpoints/bpe")
...
def main():
    ...
    for step in range(1, args.steps + 1):
        ...
        if step % args.eval_every == 0:
            val_loss = eval_loss_bpe(
                model, valid_ids, args.seq, args.batch,
