"""Training script for BPE-Bitmap + nn.Embedding LM (Experiment 3) on WikiText-2.

Hybrid input: bitmap encoder output fused with a learned token embedding via
one of three fusion methods (--fusion sum|concat|gate). Targets are the same
BPE token stream as Exp 2.

    python3 training/train_bpe_bitmap_emb.py --fusion sum
    python3 training/train_bpe_bitmap_emb.py --fusion concat
    python3 training/train_bpe_bitmap_emb.py --fusion gate
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Train BPE-Bitmap+Embedding LM on WikiText-2")

    # Fusion
    p.add_argument("--fusion", choices=["sum", "concat", "gate"], default="sum",
                   help="how to combine bitmap encoder output with token embedding")
    ...
    # tie_head=True ties the output head to token_emb.weight (standard weight tying).
    ...
    p.add_argument("--save_dir",     type=str, default=None,
                   help="default: artifacts/checkpoints/bpe_bitmap_emb_{fusion}")
...
def main():
    ...
    save_dir = Path(args.save_dir or f"artifacts/checkpoints/bpe_bitmap_emb_{args.fusion}")
    ...
    print(f"Model: {n_params/1e6:.2f}M params  "
          f"(fusion={cfg.fusion}, tie_head={cfg.tie_head})")
    ...
    for step in range(1, args.steps + 1):
        ...
        if step % args.eval_every == 0:
            ...
            if cfg.fusion == "gate":
                g = torch.sigmoid(model.fusion_gate).detach()
                print(f"  >>> gate σ(g): mean {g.mean().item():.3f}  "
                      f"min {g.min().item():.3f}  max {g.max().item():.3f}  "
                      f"(>0.5 favours bitmap stream)")
